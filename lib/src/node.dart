...
class Node {
  ...
  static final dylib = dylibLoader("std_msgs__rosidl_typesupport_c");
  ...
  factory Node.withOptions(String nodeName, String nameSpace, Context context,
    ...
    var nativeNode = malloc<rcl_node_s>()
      ..ref = rclBindings.rcl_get_zero_initialized_node();

    var rc = rclBindings.rcl_node_init(
        nativeNode,
  ...
  Publisher createPublisher() {
    var publisher = malloc<rcl_publisher_s>()
      ..ref = rclBindings.rcl_get_zero_initialized_publisher();

    var options = malloc<rcl_publisher_options_s>()
      ..ref = rclBindings.rcl_publisher_get_default_options();

    final ttt = dylib
        .lookup<NativeFunction<_typesupportFunc>>(
            'rosidl_typesupport_c__get_message_type_support_handle__std_msgs__msg__String')
        .asFunction<_typesupportFunc>();

    var rc = rclBindings.rcl_publisher_init(publisher, nativeNode, ttt(),
        "samplestrnameooo".toNativeUtf8().cast<Char>(), options);
    if (rc != RCL_RET_OK) {
...
typedef _typesupportFunc = Pointer<rosidl_message_type_support_t> Function();
